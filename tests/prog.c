...
TEST(empty_prog_is_correct) {
  const char* argv[] = {""};
  struct Program* prog = make_prog(0, argv);
...
TEST(single_file_prog_is_correct) {
  ...
  setup_tmp(fn,
      ...
      "return\n");
  const char* argv[] = {fn};
  struct Program* prog = make_prog(1, argv);
  ...
  struct SymVal val;
  struct SymKey key = mk_key("Sys.init", SBT_FUNC);
...
TEST(multi_file_prog_is_correct) {
  ...
  struct Program* prog = make_prog(3, argv);
  ...
  struct SymVal val1;
  struct SymKey key1 = mk_key("wow", SBT_LABEL);
  ...
  struct SymVal val3;
  struct SymKey key3 = mk_key("cool", SBT_LABEL);
